function cPatches = extractRandC1SoPatches(cItrainingOnly, numPatchSizes, numPatchesPerSize, patchSizes, numChannel,numPhases)
	...
	[fSiz,nfilter,filter, filters,c1OL,numSimpleFilters] = init_color_gabor_phases(rot, RF_siz, Div,numChannel,numPhases);
	fprintf(1,'done\n');
	...
	for i = 1:numPatchesPerSize
		...
		[c1source,s1source] = C1So(stim, filters, fSiz, c1SpaceSS, c1ScaleSS, c1OL,numChannel,numPhases);
		b = c1source{1};
		bsize(1) = size(b,1);
		bsize(2) = size(b,2);
		for j = 1:numPatchSizes,
