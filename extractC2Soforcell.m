function mC2 = extractC2Soforcell(filters,fSiz,c1SpaceSS,c1ScaleSS,c1OL,cPatches,cImages,numPatchSizes,numChannel,numPhases)
	%
	...
	for i = 1:numPatchSizes,
		...
		siz = sqrt(siz/(numChannel*numPhases*2));
		for j = 1:numpatch,
			tmp = reshape(cPatches{i}(:,j),[siz,siz,numChannel,2,numPhases]);
    ...
    for i = 1:nImages
        ...
        for j = 1:numPatchSizes, %for every unique patch size
            ...
            if isempty(c1),  %compute C2So
                [tmpC2,s2,c1] = C2So(stim, filters,fSiz,c1SpaceSS,c1ScaleSS,c1OL,cPatches{j},numChannel,numPhases);
            else
